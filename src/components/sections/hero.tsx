  ...
  return (
    <section className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-zinc-950 px-6 pt-16">
      ...
      <div className="absolute inset-0 z-0 flex items-center justify-center pointer-events-none">
        <div className="absolute top-1/4 left-1/4 h-[400px] w-[400px] rounded-full bg-emerald-500/20 blur-[120px]" />
        <div className="absolute bottom-1/4 right-1/4 h-[400px] w-[400px] rounded-full bg-cyan-500/20 blur-[120px]" />
      </div>

      <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col lg:flex-row items-center justify-between gap-12 lg:gap-8 pt-10 pb-20 h-full min-h-[80vh]">
        
        {/* Left/Bottom Content: Text & Buttons */}
        <div className="flex w-full lg:w-[45%] flex-col items-end lg:items-start z-20 order-2 lg:order-1 mt-[70vw] sm:mt-[60vw] md:mt-[50vw] lg:mt-0">
          
          {/* Slanted Title */}
          <div className="flex flex-col items-start gap-4 -mr-6 lg:mr-0 z-20">
            {titleLines.map((line, i) => (
              <div key={i} className={`relative inline-block pl-8 pr-4 py-1 sm:py-2 lg:pr-8 lg:translate-x-0 ${i === 0 ? 'translate-x-6' : ''}`}>
                {/* Slanted background */}
                <div className="absolute inset-y-0 left-0 w-[100vw] lg:w-full bg-white -skew-x-[15deg] shadow-2xl z-0" />
                {/* Text */}
                <span className="relative z-10 block text-2xl sm:text-3xl md:text-5xl lg:text-5xl font-black tracking-tight text-zinc-950 whitespace-nowrap">
                  {line}
                </span>
              </div>
            ))}
          ...
          {/* Buttons */}
          <div className="flex flex-row items-center justify-end lg:justify-start gap-4 mt-10 w-full">
            <div className="relative group flex-1 lg:flex-none p-[2px] hover:p-[3px] transition-all duration-200 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500">
              <a
                ...
                className="relative flex items-center justify-center gap-2 overflow-hidden rounded-[10px] bg-zinc-950 px-4 sm:px-6 py-3 font-semibold text-white transition-all active:scale-95"
              >
        ...
        <div className="absolute lg:relative top-10 lg:top-auto right-[-10%] lg:right-[-5%] w-[120%] lg:w-[75%] flex justify-end order-1 lg:order-2 z-10 pointer-events-none lg:scale-[1.25] lg:translate-x-24">
          <Image 
            src="/main-visual.png" 
