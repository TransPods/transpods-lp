    ...
    <section className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-zinc-950 px-6 pt-16">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col lg:flex-row items-center justify-between gap-12 lg:gap-8 pt-10 pb-20 h-full min-h-[80vh] lg:-translate-x-16">
        ...
        <motion.div 
          ...
          className="flex w-full lg:w-[45%] flex-col items-end lg:items-start z-20 order-2 lg:order-1 mt-[70vw] sm:mt-[60vw] md:mt-[50vw] lg:mt-64 lg:translate-x-32"
        >
          
          {/* Slanted Title */}
          <div className="flex flex-col items-start lg:items-stretch gap-4 -mr-6 lg:mr-0 z-20 lg:w-max">
            {titleLines.map((line, i) => (
              <motion.div key={i} variants={itemVariants} className={`relative inline-block lg:flex lg:justify-end pl-8 pr-4 py-1 sm:py-2 lg:pr-8 ${i === 0 ? 'translate-x-6 lg:translate-x-12' : 'lg:translate-x-0'}`}>
                {/* Slanted background */}
          ...
          {/* Subtitle */}
          <motion.p variants={itemVariants} className="mt-6 text-[13px] sm:text-sm md:text-base lg:text-lg text-zinc-300 font-medium tracking-wide text-right lg:text-left drop-shadow-md">
            {hero.subtitle}
          </motion.p>
